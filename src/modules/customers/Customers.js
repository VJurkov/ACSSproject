...
import { TableContainer, Table, TableRow, TableCell, Paper, makeStyles, TableHead, TableBody, LinearProgress, TextField } from '@material-ui/core';
    ...
    if (isLoading && customers.length === 0) {
        ...
        return (
            <div className={classes.tableContainer}>
                <TextField id="outlined-basic" label="Search" onChange={(event)=>{dispatch(setSearchTermAction(event.target.value))}}  variant="filled" fullWidth />
                <PaginationTable
                    data={customers}
                    itemsPerPage={15}
                    renderItem={(item) => {
